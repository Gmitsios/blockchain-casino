...
class App extends Component {
  ...
  enterLottery = async (lottery_address) => {
    ...
    await lottery.methods.enter().send({from: this.web3.currentProvider.selectedAddress, value: value});
  ...
  getSumInPLay = async (lottery_address) => {
    let lottery = await this.getLottery(lottery_address);
    let sum_in_play = await this.web3.eth.getBalance(lottery.options.address);
